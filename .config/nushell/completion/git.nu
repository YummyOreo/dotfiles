...
export extern "git worktree" [
    ...
    list # list of all worktrees
    lock # lock a worktree
    move # move a worktree
    prune # prune a worktree
    remove # delete a worktree
]

export extern "git worktree add" [
    path: string # the path to make it
    commit?: string # the commit to checkout into
    --force(-f) # force to create worktree
    --detach(-d) # detach HEAD in a new worktree
    -b: string # also create a new branch with given name
    --lock # lock the worktree after creation
]

export extern "git worktree list" [
    --verbose(-v) # output additional information about worktrees
    --porcelain # output in an easy-to-parse format for scripts
 ]

export extern "git worktree lock" [
    worktree: string # worktree to lock
    --reason: string # add an explanation why the working tree is locked
 ]

export extern "git worktree move" [
    worktree: string # worktree to move
    newpath: string # place to move it
 ]

export extern "git worktree prune" [
    --dry-run(-n) # Don't remove anything, just report that it would be
    --verbose(-v) # report all removals
    --expire: string
 ]

export extern "git worktree remove" [
    worktree: string # worktree to remove
    --force(-f) # force to remove worktree
 ]

export extern "git worktree repair" [
    path # path to repair
 ]

export extern "git worktree unlock" [
    worktree: string # worktree to unlock
 ]
